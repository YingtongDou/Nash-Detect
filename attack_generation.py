...
if __name__ == '__main__':
	...
	if attack == 'IncBP':
		added_edges, _, _ = bp_evasion(user_product_graph, product_user_graph, new_priors, c, r, t)
	elif attack == 'IncDS':
		added_edges, _ = ds_evasion(user_product_graph, product_user_graph, c, r, t)
	elif attack == 'IncPR':
		added_edges, _ = pr_evasion(user_product_graph, product_user_graph, priors, c, r, t, feature_config)
	elif attack == 'Random':
		...
		for index, prod in enumerate(t):
			for account in c[r*index:r*(index+1)]:
				added_edges.append((account, item))

	# save the generated reviews for corresponding attack
	with open('PostedReviews/' + dataset + '/' + attack + '.pickle', 'wb') as f:
